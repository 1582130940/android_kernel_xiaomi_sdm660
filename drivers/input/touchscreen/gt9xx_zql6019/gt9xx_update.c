...
static unsigned int booting_into_chargingmode;
char bootmode[10];
static int __init get_boot_mode(char* str) {
	printk(KERN_WARNING "getbootmode is %s\n", str);
	if ((strcmp("charger", str) == 0) || (strcmp("factory", str) == 0)) {
		booting_into_chargingmode = 1;
	}
	strcpy(bootmode, str);
	printk(KERN_WARNING "booting_into_chargingmode is %d\n", booting_into_chargingmode);
	return 0;
}
...
static s32 gup_init_panel(struct goodix_ts_data* ts) {
	...
	if (ret == SUCCESS) {
		dev_dbg(&ts->client->dev, "CFG_GROUP%d Config Version: %d, IC Config Version: %d",
...
static u8 gup_get_ic_fw_msg(struct i2c_client* client) {
	...
	dev_dbg(&client->dev, "IC Hardware info:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0],
...
s32 gup_enter_update_mode(struct i2c_client* client) {
	...
	while (retry++ < 200) {
		...
		dev_dbg(&client->dev, "Hold ss51 & dsp confirm 0x4180 failed,value:%d",
...
static u8 gup_enter_update_judge(struct i2c_client* client, struct st_fw_head* fw_head) {
	...
	if (update_msg.fw_total_len != fw_len) {
		...
		dev_err(&client->dev, " Default size: %d(%dK), actual size: %d(%dK)", fw_len, fw_len / 1024,
				update_msg.fw_total_len, update_msg.fw_total_len / 1024);
		return FAIL;
	}
	dev_info(&client->dev, "Firmware length:%d(%dK)", update_msg.fw_total_len,
...
static u8 gup_check_firmware_name(struct i2c_client* client, u8** path_p) {
	u8 len;
	u8* fname;
...
static u8 gup_get_update_file(struct i2c_client* client, struct st_fw_head* fw_head, u8* path) {
	...
	if (booting_into_chargingmode) {
		dev_err(&client->dev, "It's in charge mode %s,not update.", bootmode);
		return FAIL;
	} else
		dev_info(&client->dev, "It's in normal mode %s\n", bootmode);
	...
	if (update_msg.fw_total_len < FW_HEAD_LENGTH + FW_SECTION_LENGTH * 4 + FW_DSP_ISP_LENGTH +
										  FW_DSP_LENGTH + FW_BOOT_LENGTH) {
		dev_err(&client->dev, "INVALID bin file(size: %d), update aborted.",
	...
	dev_dbg(&client->dev, "Bin firmware actual size: %d(%dK)", update_msg.fw_total_len,
			update_msg.fw_total_len / 1024);
...
static u8 gup_burn_proc(struct i2c_client* client, u8* burn_buf, u16 start_addr, u16 total_length) {
	...
	while (burn_length < total_length) {
		...
		frame_length = ((total_length - burn_length) > PACK_SIZE) ? PACK_SIZE
...
static u8 gup_recall_check(struct i2c_client* client, u8* chk_src, u16 start_rd_addr,
	...
	while (recall_length < chk_length) {
		frame_length = ((chk_length - recall_length) > PACK_SIZE) ? PACK_SIZE
...
static u8 gup_burn_fw_section(struct i2c_client* client, u8* fw_section, u16 start_addr,
	...
	if (FAIL == ret) {
		dev_err(&client->dev, "[burn_fw_section]recall check %dk firmware fail.",
...
static u8 gup_burn_dsp_isp(struct i2c_client* client) {
	...
	while (retry++ < 5) {
		...
		if (fw_dsp_isp == NULL) {
			...
			dev_info(&client->dev, "[burn_dsp_isp]Alloc %dk byte memory success.",
...
static u8 gup_burn_fw_ss51(struct i2c_client* client) {
	...
	while (retry++ < 5) {
		...
		if (fw_ss51 == NULL) {
			...
			dev_dbg(&client->dev, "[burn_fw_ss51]Alloc %dk byte memory success.",
					(FW_SECTION_LENGTH / 1024));
			break;
		}
	}
...
static u8 gup_burn_fw_boot_isp(struct i2c_client* client) {
	...
	dev_info(&client->dev, "[burn_fw_boot_isp]Begin burn boot_isp firmware---->>");
	...
	while (retry++ < 5) {
		...
		if (fw_boot_isp == NULL) {
			...
			dev_dbg(&client->dev, "[burn_fw_boot_isp]Alloc %dk byte memory success.",
...
static u8 gup_burn_fw_link(struct i2c_client* client) {
	...
	while (retry++ < 5) {
		...
		if (fw_link == NULL) {
			...
			dev_dbg(&client->dev, "[burn_fw_link]Alloc %dk byte memory success.",
					(FW_SECTION_LENGTH / 1024));
			break;
		}
	}
	...
	ret = gup_burn_fw_gwake_section(client, fw_link, 0x9000, FW_GLINK_LENGTH - FW_SECTION_LENGTH,
									0x39);
...
static u8 gup_burn_fw_gwake(struct i2c_client* client) {
	...
	u8 retry = 0;
	s32 ret = 0;
	u16 start_index = 4 * FW_SECTION_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH + FW_BOOT_ISP_LENGTH +
	...
	while (retry++ < 5) {
		...
		if (fw_gwake == NULL) {
			...
			dev_dbg(&client->dev, "[burn_fw_gwake]Alloc %dk byte memory success.",
					(FW_SECTION_LENGTH / 1024));
			break;
		}
	}
	...
	ret = gup_load_section_file(fw_gwake, start_index + FW_SECTION_LENGTH, FW_SECTION_LENGTH,
								SEEK_SET);
	if (FAIL == ret) {
	...
	ret = gup_load_section_file(fw_gwake, start_index + 2 * FW_SECTION_LENGTH, FW_SECTION_LENGTH,
								SEEK_SET);
	if (FAIL == ret) {
	...
	ret = gup_load_section_file(fw_gwake, start_index + 3 * FW_SECTION_LENGTH, FW_SECTION_LENGTH,
								SEEK_SET);
	if (FAIL == ret) {
...
static u8 gup_burn_fw_finish(struct i2c_client* client) {
	...
	while (retry++ < 5) {
		...
		if (fw_ss51 == NULL) {
			...
			dev_dbg(&client->dev, "[burn_fw_finish]Alloc %dk byte memory success.",
					(FW_SECTION_LENGTH / 1024));
			break;
		}
	}
	...
	if (chr_status >= 1) { /* charger plugged in */
		if (!chr_pluggedin || dir_update) {
			...
			if (gtp_send_chr_cfg(i2c_connect_client) < 0) {
				...
				GTP_DEBUG("Send charger config success.");
				chr_pluggedin = 1;
			}
		}
		...
		if (chr_pluggedin || dir_update) {
			...
			if (gtp_send_cfg(i2c_connect_client) < 0) {
				...
				GTP_DEBUG("Send normal config success.");
				chr_pluggedin = 0;
			}
		}
	}
...
s32 gup_charger_proc(void* s) {
	...
	init_waitqueue_head(&ts->chaege_mode);
	ts->charge_flag = 1;
	while (1) {
		ret = wait_event_interruptible(ts->chaege_mode, ts->charge_flag);
		msleep(50);
		if (update_flag == 4)
			gtp_charger_updateconfig1(ts, 0);
	}
	return 0;
}
...
u8 gup_init_charger_proc(struct goodix_ts_data* ts) {
	...
	dev_info(&ts->client->dev, "Ready to run charger thread.");
	thread = kthread_run(gup_charger_proc, (void*)NULL, "goodix_charger_thread");

	if (IS_ERR(thread)) {
		dev_err(&ts->client->dev, "Failed to create charger thread.\n");
		return -EPERM;
	}
