...
u8 config_charger[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH] = {GTP_REG_CONFIG_DATA >> 8,
...
static inline int gtp_i2c_read(struct i2c_client* client, u8* buf, int len) {
	...
	if (likely(len < sizeof(get_buf))) {
		...
		msgs[1].buf =
				kzalloc(len > I2C_MAX_TRANSFER_SIZE ? I2C_MAX_TRANSFER_SIZE : len, GFP_KERNEL);
		if (!msgs[1].buf)
			return -ENOMEM;
	}
...
static int gtp_gesture_handler(struct goodix_ts_data* ts) {
	...
	if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') || (doze_buf[2] == 'c') ||
		...
		(doze_buf[2] == 0x5E) || (doze_buf[2] == 0xAA) || (doze_buf[2] == 0xAB) ||
		(doze_buf[2] == 0xBA) || (doze_buf[2] == 0xBB) || (doze_buf[2] == 0xCC)) {
 ...
 *
 */
static inline u8 gtp_get_points(struct goodix_ts_data* ts, struct goodix_point_t* points) {
	int ret;
	int i;
	u8* coor_data;
	u8 finger_state;
	...
	u8 point_data[2 + 1 + 8 * GTP_MAX_TOUCH_ID + 1] = {GTP_READ_COOR_ADDR >> 8,
	...
	if (touch_num > 1) {
		u8 buf[8 * GTP_MAX_TOUCH_ID] = {(GTP_READ_COOR_ADDR + 10) >> 8,
...
static irqreturn_t gtp_irq_handler(int irq, void* dev_id) {
	...
	for (i = 0, index = 0; i < ts->pdata->max_touch_id; i++) {
		if ((point_state & 0x0f) && i == points[index].id) {
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, points[index].x);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, points[index].y);
...
static s8 gtp_enter_sleep(struct goodix_ts_data* ts) {
	s8 ret;
	u8 i2c_control_buf[3] = {(u8)(GTP_REG_COMMAND >> 8), (u8)GTP_REG_COMMAND, 5};
...
static int gtp_find_valid_cfg_data(struct goodix_ts_data* ts) {
	...
	u8 cfg_info_charger_len[] = {
			CFG_GROUP_LEN(cfg_info_charger_group0), CFG_GROUP_LEN(cfg_info_charger_group1),
			CFG_GROUP_LEN(cfg_info_charger_group2), CFG_GROUP_LEN(cfg_info_charger_group3),
			CFG_GROUP_LEN(cfg_info_charger_group4), CFG_GROUP_LEN(cfg_info_charger_group5)};
#endif
	...
	u8 cfg_info_len[] = {CFG_GROUP_LEN(cfg_info_group0), CFG_GROUP_LEN(cfg_info_group1),
						 CFG_GROUP_LEN(cfg_info_group2), CFG_GROUP_LEN(cfg_info_group3),
						 CFG_GROUP_LEN(cfg_info_group4), CFG_GROUP_LEN(cfg_info_group5)};

	dev_dbg(&ts->client->dev, "Config Groups\' Lengths: %d, %d, %d, %d, %d, %d", cfg_info_len[0],
			cfg_info_len[1], cfg_info_len[2], cfg_info_len[3], cfg_info_len[4], cfg_info_len[5]);
#endif
	cfg->length = 0;
	/* read sensor id */
	...
	if ((!cfg_info_len[1]) && (!cfg_info_len[2]) && (!cfg_info_len[3]) && (!cfg_info_len[4]) &&
		(!cfg_info_len[5])) {
		sensor_id = 0;
	}
...
static ssize_t gtp_config_read_proc(struct file* file, char __user* page, size_t size,
									loff_t* ppos) {
	int i, ret;
	char* ptr;
	size_t data_len = 0;
	char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {(u8)(GTP_REG_CONFIG_DATA >> 8),
...
static ssize_t gtp_config_write_proc(struct file* filp, const char __user* buffer, size_t count,
									 loff_t* off) {
	u8* temp_buf;
	u8* file_config;
	int file_cfg_len;
	...
	file_cfg_len = gtp_ascii_to_array(temp_buf, (int)count, &file_config[GTP_ADDR_LENGTH]);
	if (file_cfg_len < 0) {
...
static ssize_t gtp_workmode_show(struct device* dev, struct device_attribute* attr, char* buf) {
	size_t data_len = 0;
...
static ssize_t gtp_dofwupdate_store(struct device* dev, struct device_attribute* attr,
...
static ssize_t gtp_drv_irq_store(struct device* dev, struct device_attribute* attr, const char* buf,
								 size_t count) {
...
static ssize_t gtp_reset_store(struct device* dev, struct device_attribute* attr, const char* buf,
							   size_t count) {
	struct goodix_ts_data* data = dev_get_drvdata(dev);
...
static int gtp_proc_init(struct kobject* sysfs_node_parent) {
	...
	sprintf(driver_path, "/sys%s", kobject_get_path(sysfs_node_parent, GFP_KERNEL));
...
static int gtp_pinctrl_init(struct goodix_ts_data* ts) {
	...
	if (IS_ERR_OR_NULL(pinctrl->default_sta)) {
		dev_info(&ts->client->dev, "Failed get pinctrl state:default state\n");
	...
	if (IS_ERR_OR_NULL(pinctrl->int_out_high)) {
		dev_info(&ts->client->dev, "Failed get pinctrl state:output_high\n");
		goto exit_pinctrl_init;
	}
...
static int gtp_request_irq(struct goodix_ts_data* ts) {
	...
	dev_info(&ts->client->dev, "INT num %d, trigger type:%d\n", ts->client->irq,
			 ts->pdata->irq_flags);
	ret = request_threaded_irq(ts->client->irq, NULL, gtp_irq_handler,
							   ts->pdata->irq_flags | IRQF_ONESHOT, ts->client->name, ts);
	if (ret < 0) {
...
static void gtp_parse_dt_coords(struct device* dev, struct goodix_ts_platform_data* pdata) {
	...
	dev_info(dev, "touch input parameters is [id x y w]<%d %d %d %d>\n", pdata->max_touch_id,
			 pdata->abs_size_x, pdata->abs_size_y, pdata->max_touch_width);
}
...
#ifdef CONFIG_TOUCHSCREEN_GT9XX_UPDATE
	if (NULL ==
		strnstr(saved_command_line, "androidboot.mode=ffbm-01", strlen(saved_command_line))) {
...
static int gtp_init_ext_watchdog(struct i2c_client* client) {
	int ret;
	u8 opr_buffer[3] = {(u8)(GTP_REG_ESD_CHECK >> 8), (u8)GTP_REG_ESD_CHECK,
...
static void gtp_esd_check_func(struct work_struct* work) {
	...
	if (test_bit(SLEEP_MODE, &ts->flags) || test_bit(FW_UPDATE_RUNNING, &ts->flags)) {
...
static void gtp_charger_check_func(struct work_struct* work) {
	...
	struct goodix_ts_charger* ts_charger =
			container_of(dwork, struct goodix_ts_charger, delayed_work);
