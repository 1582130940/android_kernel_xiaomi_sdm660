...
u8 config_charger[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH] = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
...
/*
 * Function: Goodix touchscreen sensor report function
...
/*
 * Function: Timer interrupt service routine for polling mode.
 * Input: timer: timer struct pointer
 * Output: Timer work mode.
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->driver_send_cfg) {
		...
		ret = gtp_i2c_read(ts->client,
						   cfg->data, cfg->length + GTP_ADDR_LENGTH);
		if (ret < 0)
...
static ssize_t gt9xx_enable_dt2w_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	const char c = GTP_gesture_func_on ? '1' : '0';
	return sprintf(buf, "%c\n", c);
}

static ssize_t gt9xx_enable_dt2w_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int i;
