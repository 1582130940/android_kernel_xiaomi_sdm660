...
#define GTP_VDD 33
#define GTP_TP_DATA_DUMP "goodix_rawdata_temp"
#define DEFAULT_TEST_ITEMS \
    (_MAX_TEST | _MIN_TEST | _KEY_MAX_TEST | _KEY_MIN_TEST /*| _UNIFORMITY_TEST*/)
u8 parse_config[250] = {(u8)(GTP_REG_CONFIG_DATA >> 8), (u8)GTP_REG_CONFIG_DATA, 0};
u16 max_limit_value = 5860;
u16 min_limit_value = 1182;
...
static u16 gt9xx_pixel_cnt = MAX_DRIVER_NUM * MAX_SENSOR_NUM;
static u16 gt9xx_sc_pxl_cnt = MAX_DRIVER_NUM * MAX_SENSOR_NUM;
...
static unsigned short gt900_short_threshold = 10;
static unsigned short gt900_resistor_threshold = 800;

static unsigned short gt900_gnd_resistor_threshold = 500;
...
#define SET_INFO_LINE_INFO(fmt, args...)							  \
	do {															  \
...
#define SET_INFO_LINE_ERR(fmt, args...)								\
	do {															   \
		memset(tmp_info_line, '\0', 80);							   \
...
static s32 gtp_parse_config(struct i2c_client* client) {
	...
	if (gtp_i2c_read(client, parse_config, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH) <= 0) {
	...
	gt9xx_drv_num = (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT - GT9_REG_CFG_BEG] & 0x1F) +
					(config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 1 - GT9_REG_CFG_BEG] & 0x1F);
	gt9xx_sen_num =
			(config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 2 - GT9_REG_CFG_BEG] & 0x0F) +
			((config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 2 - GT9_REG_CFG_BEG] >> 4) & 0x0F);
	...
	GTP_DEBUG("drv num: %d, sen num: %d, sc drv num: %d", gt9xx_drv_num, gt9xx_sen_num,
			  gt9xx_sc_drv_num);

	if (key_is_isolated) {
		GTP_DEBUG("Isolated [%d key(s)]: %d, %d, %d, %d", key_iso_pos[0], key_iso_pos[1],
				  key_iso_pos[2], key_iso_pos[3], key_iso_pos[4]);
	}
	...
	while (left > 0) {
		...
		for (i = 0; i < opr_len; ++i) {
			if (opr_buf[i + 2] != dsp_short[addr - GTP_REG_DSP_SHORT + i]) {
				...
				GTP_DEBUG("(%d)Location: %d, 0x%02X, 0x%02X", retry + 1,
						  addr - GTP_REG_DSP_SHORT + i, opr_buf[i + 2],
...
static s32 gtp_compute_rslt(struct i2c_client* client) {
	...
	short_sum = (struct gt9xx_short_info*)kzalloc(sizeof(struct gt9xx_short_info) * _SHORT_INFO_MAX,
												  GFP_KERNEL);
	...
	if (node_idx == 0) {
		...
		for (i = 0, j = 0; i < node_idx; ++i) {
			GTP_DEBUG("Orignal Shorted Channels: %s%d, %s%d",
					  ...
					  short_sum[i].master & (~GT9_DRV_HEAD),
					  (short_sum[i].slave_is_driver) ? "Drv" : "Sen",
					  short_sum[i].slave & (~GT9_DRV_HEAD));
			...
			if (master == 0xFF && slave == 0xFF) {
				GTP_DEBUG("unbonded channel (%d, %d) shorted!", short_sum[i].master,
						  short_sum[i].slave);
				continue;
			} else {
		...
		if (node_idx == 0) {
			...
			for (i = 0; i < node_idx; ++i) {
				SET_INFO_LINE_INFO("  %s%02d & %s%02d Shorted! (R = %dKOhm)",
								   ...
								   short_sum[i].master,
								   (short_sum[i].slave_is_driver) ? "Drv" : "Sen",
								   short_sum[i].slave, short_sum[i].impedance);
			}
			...
			if (short_res < (gt900_gnd_resistor_threshold * amplifier)) {
				if (i < MAX_DRIVER_NUM * 2) {
					...
					if (short_chnl == 0xFF) {
						...
						SET_INFO_LINE_INFO("  Drv%02d & GND/VDD Shorted! (R = %dKOhm)", short_chnl,
										   short_res / amplifier);
					}
				} else {
					...
					if (short_chnl == 0xFF) {
						...
						SET_INFO_LINE_INFO("  Sen%02d & GND/VDD Shorted! (R = %dKOhm)", short_chnl,
										   short_res / amplifier);
					}
				}
...
static s32 gtp_read_rawdata(struct i2c_client* client, u16* data) {
	...
	read_rawbuf = (u8*)kzalloc(sizeof(u8) * (gt9xx_drv_num * gt9xx_sen_num * 2 + GTP_ADDR_LENGTH),
							   GFP_KERNEL);
	...
	ret = gtp_i2c_read(client, read_rawbuf,
					   GTP_ADDR_LENGTH + ((gt9xx_drv_num * gt9xx_sen_num) * 2));
	...
	for (i = 0, j = 0; i < ((gt9xx_drv_num * gt9xx_sen_num) * 2); i += 2) {
		data[i / 2] = (u16)(read_rawbuf[i + head + GTP_ADDR_LENGTH] << 8) +
...
static s32 gtp_read_diffdata(struct i2c_client* client, u16* data) {
	s32 ret = -1;
	u16 retry = 0;
	u8 read_state[3] = {(u8)(GTP_REG_RAW_READY >> 8), (u8)GTP_REG_RAW_READY, 0};
	u16 i = 0, j = 0;
	...
	for (i = 0, j = 0; i < ((gt9xx_drv_num * gt9xx_sen_num) * 2); i += 2) {
		data[i / 2] = (int)((read_rawbuf[i + head + GTP_ADDR_LENGTH] << 8) +
							read_rawbuf[GTP_ADDR_LENGTH + i + tail]);
#if GTP_DEBUG_ARRAY_ON
		printk("%4d ", data[i / 2]);
		++j;
...
static s32 gtp_raw_test_init(void) {
	...
	touchpad_sum = (struct gt9xx_open_info*)kzalloc(
			sizeof(struct gt9xx_open_info) * (4 * _BEYOND_INFO_MAX + 1), GFP_ATOMIC);
	...
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++) {
		if (raw_buf[i] < min_limit_value) {
			...
			if (new_flag) {
				...
				GTP_DEBUG("[%d, %d]rawdata: %d, raw min limit: %d", driver, sensor, raw_buf[i],
						  min_limit_value);
			} else {
				continue;
			}
	...
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++) {
		if (raw_buf[i] > max_limit_value) {
			...
			if (new_flag) {
				...
				touchpad_sum[j].raw_val = raw_buf[i];
				touchpad_sum[j].times = 1;
				GTP_DEBUG("[%d, %d]rawdata: %d, raw max limit: %d", driver, sensor, raw_buf[i],
						  max_limit_value);
			} else {
				continue;
			}
	...
	for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i) {
		...
		if (raw_buf[i] > max_limit_key) {
			...
			if (new_flag) {
				...
				GTP_DEBUG("[%d, %d]key rawdata: %d, key max limit: %d", driver, sensor, raw_buf[i],
						  max_limit_key);
			} else {
				continue;
			}
	...
	for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i) {
		...
		if (raw_buf[i] < min_limit_key) {
			...
			if (new_flag) {
				...
				GTP_DEBUG("[%d, %d]key rawdata: %d, key min limit: %d", driver, sensor, raw_buf[i],
						  min_limit_key);
			} else {
				continue;
			}
...
static void gtp_uniformity_test(u16* raw_buf) {
	...
	if (uniformity < uniformity_lmt) {
		...
		pr_info("min_val: %d, max_val: %d, uniformity: %d%%, times: %d", min_val, max_val,
				uniformity, touchpad_sum[sum_base].times);
	}
}
...
static s32 gtp_get_test_result(void) {
...
#if GTP_DEBUG_ON
	for (i = 0; i < 4 * _BEYOND_INFO_MAX; ++i) {
		printk("(%2d, %2d)[%2d] ", touchpad_sum[i].driver, touchpad_sum[i].sensor,
	...
	if (beyond_max_num > _MIN_ERROR_NUM) {
		...
		for (i = 0; i < _BEYOND_INFO_MAX; ++i) {
			...
			SET_INFO_LINE_INFO("  Drv: %d, Sen: %d RawVal: %d", touchpad_sum[i].driver,
							   touchpad_sum[i].sensor, touchpad_sum[i].raw_val);
		}
	}
	...
	if (beyond_uniformity > _MIN_ERROR_NUM) {
		...
		SET_INFO_LINE_INFO("  Uniformity Limit: %d%%, Tp Uniformity: %d%%", uniformity_lmt,
						   touchpad_sum[4 * _BEYOND_INFO_MAX].raw_val /
								   touchpad_sum[4 * _BEYOND_INFO_MAX].times);
	}
...
static s32 gt9xx_open_test(struct i2c_client* client) {
	...
	SET_INFO_LINE_INFO("Total %d Sample Data, Max Show %d Info for each Test Item",
					   GTP_OPEN_SAMPLE_NUM, _BEYOND_INFO_MAX);
...
static ssize_t gt9xx_tp_selftest_read(struct file* file, char __user* buf, size_t count,
									  loff_t* pos) {
	char tmp[5];
	int cnt;
...
static ssize_t gt9xx_tp_selftest_write(struct file* file, const char __user* buf, size_t count,
									   loff_t* pos) {
	char tmp[6];
	int retval;
	...
	if (!strncmp(tmp, "short", 5)) {
		...
		if (gt9xx_i2c_test(i2c_connect_client) == SUCCESS)
			if (gt9xx_open_test(i2c_connect_client) == SUCCESS)
				retval = gt9xx_short_test(i2c_connect_client);
	}
...
static ssize_t gt9xx_tp_datadump_read(struct file* file, char __user* buf, size_t count,
									  loff_t* pos) {
	int ret = 0, cnt = 0;
	char* tmp = NULL;
	struct goodix_ts_data* ts = i2c_get_clientdata(i2c_connect_client);
	...
	if (tmp == NULL)
		return 0;
	disable_irq(ts->client->irq);
	cnt = gt9xx_get_and_format_rawdata(tmp, i2c_connect_client);
	if (cnt == 0)
		goto out;

	ret = gt9xx_get_and_format_diffdata(tmp + cnt, i2c_connect_client);
	if (ret == 0)
		goto out;
...
void gtp_test_sysfs_init(void) {
	gt9xx_selftest_proc = proc_create("tp_selftest", S_IRWXU, NULL, &tp_selftest_ops);
}
...
static int tp_data_dump_proc_open(struct inode* inode, struct file* file) {
	...
	u16* diffdata_buf = NULL;
	struct i2c_client* client = i2c_connect_client;
	ts = i2c_get_clientdata(client);

	GTP_DEBUG("------------------------enter gtp cat proc data-------------------");
	disable_irq(ts->client->irq);
	gtp_esd_off(ts);
	...
	rawdata_buf = (u16*)kmalloc(sizeof(u16) * (gt9xx_drv_num * gt9xx_sen_num), GFP_KERNEL);

	if (NULL == rawdata_buf) {
		GTP_DEBUG("failed to allocate mem for rawdata_buf!");
		ret = FAIL;
		goto open_test_exit;
	}
	diffdata_buf = (u16*)kmalloc(sizeof(u16) * (gt9xx_drv_num * gt9xx_sen_num), GFP_KERNEL);
	if (NULL == diffdata_buf) {
		GTP_DEBUG("failed to allocate mem for diffdata_buf!");
		ret = FAIL;
		goto open_test_exit;
	}
	...
	msleep(80);
	ret = gtp_read_diffdata(client, diffdata_buf);
	if (ret == FAIL) {
	...
	ret = Save_testing_data(diffdata_buf, 0x0000, rawdata_buf);
	if (ret < 0) {
		GTP_DEBUG("save rawdata to csv error!");
		ret = FAIL;
		goto open_test_exit;
	}
...
open_test_exit:
	...
	gtp_esd_on(ts);
	gt9_read_coor_cmd(client);
	GTP_DEBUG("------------------------leave gtp cat proc data-------------------");
