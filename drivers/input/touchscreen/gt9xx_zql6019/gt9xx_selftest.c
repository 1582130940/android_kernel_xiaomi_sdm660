...
static s32 gt9xx_short_parse_cfg(void)
{
	...
	drv_num = (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT - GT9_REG_CFG_BEG] & 0x1F) + (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 1 - GT9_REG_CFG_BEG] & 0x1F);
	sen_num = (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 2 - GT9_REG_CFG_BEG] & 0x0F) + ((config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 2 - GT9_REG_CFG_BEG] >> 4) & 0x0F);
...
/*
 * @param: phy_chnl: ic detected short channel, is_driver: it's driver or not
 * @Return: 0xff: the ic channel is not used, otherwise: the tp short channel
 */
...
static s32 gtp_parse_config(struct i2c_client *client)
{
	...
	gt9xx_drv_num = (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT - GT9_REG_CFG_BEG] & 0x1F) + (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 1 - GT9_REG_CFG_BEG] & 0x1F);
	gt9xx_sen_num = (config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 2 - GT9_REG_CFG_BEG] & 0x0F) + ((config[GTP_ADDR_LENGTH + GT9_REG_SEN_DRV_CNT + 2 - GT9_REG_CFG_BEG] >> 4) & 0x0F);
...
/*
 * Function: write one byte to specified register
 * Input: reg: the register address
 * val: the value to write into
 * Return: i2c_write function return
 */
...
/*
 * Function: read one byte from specified register into buf
 * Input: reg: the register
 * buf: the buffer for one byte
 * Return: i2c_read function return
 */
...
/*
 * Function: burn dsp_short code
 * Input: i2c_client
 * Return: SUCCESS: burning succeed, FAIL: burning failed
 */
...
/*
 * Function: check the resistor between shortlike channels if less than threshold confirm as short
 * INPUT: Short like Information struct pointer
 * Returns: SUCCESS: it's shorted FAIL: otherwise
 */
...
/*
 * Function: compute the result, whether there are shorts or not
 * Input: i2c_client
 * Return: SUCCESS
 */
...
/*
 * Function: gt9 series ic short test function
 * Input: I2c_client, i2c device
 * Return: SUCCESS: test succeed, FAIL: test failed
 */
...
/*
 * Function: send read rawdata cmd
 * Input: i2c_client* client: i2c device
 * Return: SUCCESS: send process succeed, FAIL: failed
 */
...
/*
 * Function: read rawdata from ic registers
 * Input: u16* data: rawdata buffer
 * i2c_client* client: i2c device
 * Return: SUCCESS: read process succeed, FAIL:  failed
 */
...
/*
 * Function: rawdata test initilization function
 * Input: u32 check_types: test items
 */
...
/*
 * Function: touchscreen rawdata min limit test
 * Input: u16* raw_buf: rawdata buffer
 */
...
/*
 * Function: touchscreen rawdata max limit test
 * Input: u16* raw_buf: rawdata buffer
 */
...
/*
 * Function: key rawdata max limit test
 * Input: u16* raw_buf: rawdata buffer
 */
...
/*
 * Function: key rawdata min limit test
 * Input: u16* raw_buf: rawdata buffer
 */
...
/*
 * Function: analyse rawdata retrived from ic registers
 * Input: u16 *raw_buf, buffer for rawdata,
 * u32 check_types, test items
 * Return: SUCCESS: test process succeed, FAIL: failed
 */
...
/*
 * Function: output the test result
 * Return: return the result. if result == 0, the TP is ok, otherwise list the beyonds
 */
...
/*
 * Function: test gt9 series ic open test
 * Input: client, i2c_client
 * Return: SUCCESS: test process success, FAIL, test process failed
 */
