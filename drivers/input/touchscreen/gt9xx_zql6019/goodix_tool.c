...
ssize_t goodix_tool_write(struct file* filp, const char __user* buff, size_t len, loff_t* off) {
	...
	dev_dbg(&gt_client->dev, "[Flag]flag: %02X,addr: %02X%02X,value: %02X,relation: %02X",
			cmd_head.flag, cmd_head.flag_addr[0], cmd_head.flag_addr[1], cmd_head.flag_val,
			cmd_head.flag_relation);
	dev_dbg(&gt_client->dev, "[Retry]circle: %d,times: %d,retry: %d, delay: %d",
			(s32)cmd_head.circle, (s32)cmd_head.times, (s32)cmd_head.retry, (s32)cmd_head.delay);
	...
	if (1 == cmd_head.wr) {
		...
		ret = copy_from_user(&cmd_head.data[GTP_ADDR_LENGTH], &buff[CMD_HEAD_LENGTH],
							 cmd_head.data_len);
		if (ret) {
			dev_err(&gt_client->dev, "copy_from_user failed.");
			return -EPERM;
...
ssize_t goodix_tool_read(struct file* file, char __user* page, size_t size, loff_t* ppos) {
	...
	if (cmd_head.wr % 2) {
		...
		if (cmd_head.delay) msleep(cmd_head.delay);
		...
		while (data_len > 0) {
			...
			ret = simple_read_from_buffer(&page[loc], size, ppos, &cmd_head.data[GTP_ADDR_LENGTH],
										  len);
			if (ret < 0)
				return ret;
			loc += len;
		...
		ret = simple_read_from_buffer(page, size, ppos, GTP_DRIVER_VERSION,
