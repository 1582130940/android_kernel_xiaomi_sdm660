...
static int fts_irq_registration(struct fts_ts_data* ts_data) {
	...
	ret = request_threaded_irq(ts_data->irq, NULL, fts_ts_interrupt,
							   pdata->irq_gpio_flags | IRQF_ONESHOT, ts_data->client->name,
							   ts_data);
...
static int fts_parse_dt(struct device* dev, struct fts_ts_platform_data* pdata) {
	...
	if (pdata->have_key) {
		...
		ret = of_property_read_u32_array(np, "focaltech,key-x-coords", pdata->key_x_coords,
										 pdata->key_number);
		if (ret)
			FTS_ERROR("Key X Coords undefined!");

		FTS_INFO("VK(%d): (%d, %d, %d), [%d, %d, %d][%d]", pdata->key_number, pdata->keys[0],
				 pdata->keys[1], pdata->keys[2], pdata->key_x_coords[0], pdata->key_x_coords[1],
	...
	FTS_INFO("max touch number:%d, irq gpio:%d, reset gpio:%d", pdata->max_touch_number,
...
int fts_LockDownInfo_get_from_boot(struct i2c_client* client, char* pProjectCode) {
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		sprintf(pProjectCode, "%02x%02x%02x%02x%02x%02x%02x%02x", r_buf[0], r_buf[1], r_buf[2],
