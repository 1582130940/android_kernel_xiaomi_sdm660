...
static int hall_parse_dt(struct device* dev, struct hall_switch_info* pdata) {
	...
	pdata->irq_gpio = of_get_named_gpio_flags(np, "hall,irq-gpio", 0, &pdata->irq_gpio_flags);
	if (pdata->irq_gpio < 0)
...
static ssize_t hall_state_show(struct class* class, struct class_attribute* attr, char* buf) {
	int state;
...
static struct class_attribute hall_state = __ATTR(hall_state, S_IRUGO, hall_state_show, NULL);
