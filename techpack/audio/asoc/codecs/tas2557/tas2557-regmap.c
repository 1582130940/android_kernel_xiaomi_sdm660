 ...
 */
static int tas2557_i2c_write_device(struct tas2557_priv* pTAS2557, unsigned char addr,
 ...
 * platform dependent, need platform specific support
 */
static int tas2557_i2c_bulkwrite_device(struct tas2557_priv* pTAS2557, unsigned char addr,
										unsigned char reg, unsigned char* pBuf, unsigned int len) {
	int ret = 0;
 ...
 * platform dependent, need platform specific support
 */
static int tas2557_i2c_read_device(struct tas2557_priv* pTAS2557, unsigned char addr,
 ...
 */
static int tas2557_change_book_page(struct tas2557_priv* pTAS2557, enum channel chn,
	...
	if (chn & channel_left) {
		if (pTAS2557->mnLCurrentBook == nBook) {
			if (pTAS2557->mnLCurrentPage != nPage) {
				nResult = tas2557_i2c_write_device(pTAS2557, pTAS2557->mnLAddr,
			...
			nResult =
					tas2557_i2c_write_device(pTAS2557, pTAS2557->mnLAddr, TAS2557_BOOKCTL_PAGE, 0);
			if (nResult < 0)
				goto end;
			pTAS2557->mnLCurrentPage = 0;
			nResult = tas2557_i2c_write_device(pTAS2557, pTAS2557->mnLAddr, TAS2557_BOOKCTL_REG,
											   nBook);
			if (nResult < 0)
				goto end;
	...
	if (chn & channel_right) {
		if (pTAS2557->mnRCurrentBook == nBook) {
			if (pTAS2557->mnRCurrentPage != nPage) {
				nResult = tas2557_i2c_write_device(pTAS2557, pTAS2557->mnRAddr,
												   TAS2557_BOOKCTL_PAGE, nPage);
				if (nResult < 0)
					goto end;
			...
			nResult =
					tas2557_i2c_write_device(pTAS2557, pTAS2557->mnRAddr, TAS2557_BOOKCTL_PAGE, 0);
			if (nResult < 0)
				goto end;
			pTAS2557->mnRCurrentPage = 0;
			nResult = tas2557_i2c_write_device(pTAS2557, pTAS2557->mnRAddr, TAS2557_BOOKCTL_REG,
											   nBook);
			if (nResult < 0)
				goto end;
	...
	if (chn == channel_broadcast) {
		nResult =
				tas2557_i2c_write_device(pTAS2557, TAS2557_BROADCAST_ADDR, TAS2557_BOOKCTL_PAGE, 0);
		if (nResult < 0)
			goto end;
		pTAS2557->mnLCurrentPage = 0;
		pTAS2557->mnRCurrentPage = 0;
		nResult = tas2557_i2c_write_device(pTAS2557, TAS2557_BROADCAST_ADDR, TAS2557_BOOKCTL_REG,
										   nBook);
		if (nResult < 0)
			goto end;
		pTAS2557->mnLCurrentBook = nBook;
		pTAS2557->mnRCurrentBook = nBook;
		nResult = tas2557_i2c_write_device(pTAS2557, TAS2557_BROADCAST_ADDR, TAS2557_BOOKCTL_PAGE,
										   nPage);
		if (nResult >= 0) {
			pTAS2557->mnLCurrentPage = nPage;
			pTAS2557->mnRCurrentPage = nPage;
 ...
 * platform independent, don't change unless necessary
 */
static int tas2557_dev_read(struct tas2557_priv* pTAS2557, enum channel chn, unsigned int nRegister,
							unsigned int* pValue) {
	int nResult = 0;
	...
	nResult = tas2557_change_book_page(pTAS2557, chn, TAS2557_BOOK_ID(nRegister),
									   TAS2557_PAGE_ID(nRegister));
	if (nResult < 0)
		goto end;

	if (chn == channel_left)
		nResult = tas2557_i2c_read_device(pTAS2557, pTAS2557->mnLAddr, TAS2557_PAGE_REG(nRegister),
										  &Value);
	else if (chn == channel_right)
		nResult = tas2557_i2c_read_device(pTAS2557, pTAS2557->mnRAddr, TAS2557_PAGE_REG(nRegister),
										  &Value);
	else {
 ...
 * platform independent, don't change unless necessary
 */
static int tas2557_dev_write(struct tas2557_priv* pTAS2557, enum channel chn,
	...
	if (pTAS2557->mbTILoadActive) {
		...
		dev_dbg(pTAS2557->dev, "TiLoad W CH[%d] REG B[%d]P[%d]R[%d] =0x%x\n", chn,
				TAS2557_BOOK_ID(nRegister), TAS2557_PAGE_ID(nRegister), TAS2557_PAGE_REG(nRegister),
				nValue);
	}
	...
	if (chn & channel_left) {
		nResult = tas2557_i2c_write_device(pTAS2557, pTAS2557->mnLAddr, TAS2557_PAGE_REG(nRegister),
										   nValue);
		if (nResult < 0)
			goto end;
	}
	if (chn & channel_right) {
		nResult = tas2557_i2c_write_device(pTAS2557, pTAS2557->mnRAddr, TAS2557_PAGE_REG(nRegister),
										   nValue);
		if (nResult < 0)
			goto end;
	}
	if (chn == channel_broadcast)
		nResult = tas2557_i2c_write_device(pTAS2557, TAS2557_BROADCAST_ADDR,
 ...
 * platform independent, don't change unless necessary
 */
static int tas2557_dev_bulk_read(struct tas2557_priv* pTAS2557, enum channel chn,
...
#define STRIDE 4
		...
		for (i = 0; i < (nLength / STRIDE); i++) {
			nResult =
					regmap_bulk_read(pTAS2557->mpRegmap, TAS2557_PAGE_REG((nRegister + i * STRIDE)),
		...
		if ((nLength % STRIDE) != 0) {
			nResult = regmap_bulk_read(pTAS2557->mpRegmap, TAS2557_PAGE_REG(nRegister + i * STRIDE),
 ...
 * platform independent, don't change unless necessary
 */
static int tas2557_dev_bulk_write(struct tas2557_priv* pTAS2557, enum channel chn,
								  unsigned int nRegister, u8* pData, unsigned int nLength) {
	int nResult = 0;
	unsigned char reg = 0;
	...
	if (nResult >= 0) {
		...
		if (chn & channel_left) {
			nResult =
					tas2557_i2c_bulkwrite_device(pTAS2557, pTAS2557->mnLAddr, reg, pData, nLength);
			if (nResult < 0)
				goto end;
		}
		...
		if (chn & channel_right) {
			nResult =
					tas2557_i2c_bulkwrite_device(pTAS2557, pTAS2557->mnRAddr, reg, pData, nLength);
			if (nResult < 0)
				goto end;
		}

		if (chn == channel_broadcast)
			nResult = tas2557_i2c_bulkwrite_device(pTAS2557, TAS2557_BROADCAST_ADDR, reg, pData,
												   nLength);
	}
 ...
 * platform independent, don't change unless necessary
 */
static int tas2557_dev_update_bits(struct tas2557_priv* pTAS2557, enum channel chn,
	...
	if (chn & channel_left) {
		nResult = tas2557_i2c_update_bits(pTAS2557, pTAS2557->mnLAddr, TAS2557_PAGE_REG(nRegister),
										  nMask, nValue);
		if (nResult < 0)
			goto end;
	}
	...
	if (chn & channel_right)
		nResult = tas2557_i2c_update_bits(pTAS2557, pTAS2557->mnRAddr, TAS2557_PAGE_REG(nRegister),
										  nMask, nValue);
...
static void irq_work_routine(struct work_struct* work) {
	struct tas2557_priv* pTAS2557 = container_of(work, struct tas2557_priv, irq_work.work);
	...
	if (pConfiguration->mnDevices & channel_left) {
		...
		if (((nDevLInt1Status & 0xfc) != 0) || ((nDevLInt2Status & 0x0c) != 0)) {
			...
			nCounter = 2;
			while (nCounter > 0) {
				nResult = tas2557_dev_read(pTAS2557, channel_left, TAS2557_POWER_UP_FLAG_REG,
										   &nDevLInt1Status);
				if (nResult < 0)
					goto program;
	...
	if (pConfiguration->mnDevices & channel_right) {
		...
		if (((nDevRInt1Status & 0xfc) != 0) || ((nDevRInt2Status & 0x0c) != 0)) {
			...
			nCounter = 2;
			while (nCounter > 0) {
				nResult = tas2557_dev_read(pTAS2557, channel_right, TAS2557_POWER_UP_FLAG_REG,
										   &nDevRInt1Status);
				if (nResult < 0)
					goto program;
...
static int tas2557_i2c_probe(struct i2c_client* pClient, const struct i2c_device_id* pID) {
	...
	atomic_set(&tas2557_ref_count, 0);
	...
	if (gpio_is_valid(pTAS2557->mnLeftChlGpioRst) || gpio_is_valid(pTAS2557->mnRightChlGpioRst))
	...
	if (pTAS2557->mnLPGID != pTAS2557->mnRPGID) {
		dev_err(pTAS2557->dev,
				"HardWare Critical: L-PGID=0x%x, R-PGID=0x%x, please use same version\n",
	...
	if (gpio_is_valid(pTAS2557->mnLeftChlGpioINT)) {
		...
		if (nResult < 0) {
			dev_err(pTAS2557->dev, "%s: GPIO %d request INT error\n", __func__,
		...
		nResult = request_threaded_irq(pTAS2557->mnLeftChlIRQ, tas2557_irq_handler, NULL,
									   IRQF_TRIGGER_HIGH | IRQF_ONESHOT, pClient->name, pTAS2557);
		if (nResult < 0) {
	...
	if (gpio_is_valid(pTAS2557->mnRightChlGpioINT)) {
		if (pTAS2557->mnLeftChlGpioINT != pTAS2557->mnRightChlGpioINT) {
			...
			nResult =
					request_threaded_irq(pTAS2557->mnRightChlIRQ, tas2557_irq_handler, NULL,
										 IRQF_TRIGGER_HIGH | IRQF_ONESHOT, pClient->name, pTAS2557);
			if (nResult < 0) {
				dev_err(pTAS2557->dev, "request_irq failed, %d\n", nResult);
				goto err;
	...
	INIT_WORK(&pTAS2557->mtimerwork, timer_work_routine);
		...
		nResult = request_firmware_nowait(THIS_MODULE, 1, TAS2557_FW_NAME, pTAS2557->dev,
