...
static unsigned int tas2557_codec_read(struct snd_soc_component* pCodec, unsigned int nRegister) {
	...
	ret = pTAS2557->read(pTAS2557, pTAS2557->mnCurrentChannel, nRegister, &Value);
	if (ret < 0)
...
static int tas2557_codec_write(struct snd_soc_component* pCodec, unsigned int nRegister,
	...
	ret = pTAS2557->write(pTAS2557, pTAS2557->mnCurrentChannel, nRegister, nValue);
...
static int tas2557_startup(struct snd_pcm_substream* substream, struct snd_soc_dai* dai) {
...
static void tas2557_shutdown(struct snd_pcm_substream* substream, struct snd_soc_dai* dai) {
	struct snd_soc_component* component = dai->component;
	struct tas2557_priv* pTAS2557 = snd_soc_component_get_drvdata(component);
...
static int tas2557_set_dai_sysclk(struct snd_soc_dai* pDAI, int nClkID, unsigned int nFreqency,
								  int nDir) {
...
static int tas2557_prepare(struct snd_pcm_substream* pSubstream, struct snd_soc_dai* pDAI) {
	struct snd_soc_component* component = pDAI->component;
	struct tas2557_priv* pTAS2557 = snd_soc_component_get_drvdata(component);
...
static int tas2557_set_bias_level(struct snd_soc_component* pCodec, enum snd_soc_bias_level eLevel) {
	struct tas2557_priv* pTAS2557 = snd_soc_component_get_drvdata(pCodec);
...
static int tas2557_fs_get(struct snd_kcontrol* pKcontrol, struct snd_ctl_elem_value* pValue) {
#ifdef KCONTROL_CODEC
	struct snd_soc_component* component = snd_soc_kcontrol_component(pKcontrol);
#else
	struct snd_soc_component* component = snd_kcontrol_chip(pKcontrol);
	...
	if (pTAS2557->mpFirmware->mnConfigurations)
		nFS = pTAS2557->mpFirmware->mpConfigurations[pTAS2557->mnCurrentConfiguration]
					  .mnSamplingRate;
...
static int tas2557_fs_put(struct snd_kcontrol* pKcontrol, struct snd_ctl_elem_value* pValue) {
#ifdef KCONTROL_CODEC
	struct snd_soc_component* component = snd_soc_kcontrol_component(pKcontrol);
#else
	struct snd_soc_component* component = snd_kcontrol_chip(pKcontrol);
...
static int tas2557_program_get(struct snd_kcontrol* pKcontrol, struct snd_ctl_elem_value* pValue) {
#ifdef KCONTROL_CODEC
	struct snd_soc_component* component = snd_soc_kcontrol_component(pKcontrol);
#else
	struct snd_soc_component* component = snd_kcontrol_chip(pKcontrol);
...
static int tas2557_program_put(struct snd_kcontrol* pKcontrol, struct snd_ctl_elem_value* pValue) {
#ifdef KCONTROL_CODEC
	struct snd_soc_component* component = snd_soc_kcontrol_component(pKcontrol);
#else
	struct snd_soc_component* component = snd_kcontrol_chip(pKcontrol);
...
static int tas2557_echoref_ctl_put(struct snd_kcontrol* pKcontrol,
	...
	struct tas2557_priv* pTAS2557 = snd_soc_component_get_drvdata(component);
	int echoref =
			pValue->value.integer.value[0] & 0x01; /* only take care of left/right channel switch */
...
static const struct snd_kcontrol_new tas2557_snd_controls[] = {
	...
	SOC_SINGLE_EXT("Stereo PowerCtrl", SND_SOC_NOPM, 0, 0x0001, 0, tas2557_power_ctrl_get,
				   tas2557_power_ctrl_put),
	SOC_SINGLE_EXT("Stereo Program", SND_SOC_NOPM, 0, 0x00FF, 0, tas2557_program_get,
				   tas2557_program_put),
	SOC_SINGLE_EXT("Stereo Configuration", SND_SOC_NOPM, 0, 0x00FF, 0,
				   tas2557_configuration_get, tas2557_configuration_put),
	SOC_SINGLE_EXT("Stereo FS", SND_SOC_NOPM, 8000, 48000, 0, tas2557_fs_get, tas2557_fs_put),
	SOC_SINGLE_EXT("Get DevA Cali_Re", SND_SOC_NOPM, 0, 0x7f000000, 0, tas2557_DevA_Cali_get,
				   NULL),
	SOC_SINGLE_EXT("Get DevB Cali_Re", SND_SOC_NOPM, 0, 0x7f000000, 0, tas2557_DevB_Cali_get,
				   NULL),
	SOC_SINGLE_EXT("Stereo Calibration", SND_SOC_NOPM, 0, 0x00FF, 0, tas2557_calibration_get,
				   tas2557_calibration_put),
	SOC_ENUM_EXT("Stereo DSPChl Setup", chl_setup_enum[0], tas2557_dsp_chl_setup_get,
				 tas2557_dsp_chl_setup_put),
	SOC_ENUM_EXT("VBoost Ctrl", vboost_ctl_enum[0], tas2557_vboost_ctl_get,
				 tas2557_vboost_ctl_put),
	SOC_ENUM_EXT("VBoost Volt", vboost_volt_enum[0], tas2557_vboost_volt_get,
				 tas2557_vboost_volt_put),
	SOC_ENUM_EXT("Stereo EchoRef Ctrl", echoref_ctl_enum[0], tas2557_echoref_ctl_get,
				 tas2557_echoref_ctl_put),
};
...
int tas2557_register_codec(struct tas2557_priv* pTAS2557) {
	...
	nResult = snd_soc_register_component(pTAS2557->dev, &soc_component_driver_tas2557, tas2557_dai_driver,
