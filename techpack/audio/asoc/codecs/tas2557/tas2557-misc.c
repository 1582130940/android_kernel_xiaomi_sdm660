...
static ssize_t tas2557_file_read(struct file* file, char* buf, size_t count, loff_t* ppos) {
	...
	switch (pTAS2557->mnDBGCmd) {
		case TIAUDIO_CMD_REG_READ: {
			if (g_logEnable)
				dev_info(pTAS2557->dev,
						 "TIAUDIO_CMD_REG_READ: chn[%d], current_reg = 0x%x, count=%d\n",
						 pTAS2557->mnCurrentChannel, pTAS2557->mnCurrentReg, (int)count);
			if (count == 1) {
				ret = pTAS2557->read(pTAS2557, pTAS2557->mnCurrentChannel, pTAS2557->mnCurrentReg,
									 &nValue);
				if (ret < 0) {
			...
			if ((pTAS2557->mpFirmware->mnConfigurations > 0) &&
				...
				if (count == PROGRAM_BUF_SIZE) {
					...
					if (p_kBuf != NULL) {
						...
						strlcpy(&p_kBuf[5 + FW_NAME_SIZE], pProgram->mpDescription,
								strlen(pProgram->mpDescription) + 1);
						ret = copy_to_user(buf, p_kBuf, count);
						if (ret != 0) {
							/* Failed to copy all the data, exit */
							dev_err(pTAS2557->dev, "copy to user fail %d\n", ret);
				...
				if (count == CONFIGURATION_BUF_SIZE) {
					...
					if (p_kBuf != NULL) {
						...
						p_kBuf[5 + FW_NAME_SIZE] =
								((pConfiguration->mnSamplingRate & 0x0000ff00) >> 8);
						p_kBuf[6 + FW_NAME_SIZE] =
								((pConfiguration->mnSamplingRate & 0x00ff0000) >> 16);
						p_kBuf[7 + FW_NAME_SIZE] =
								((pConfiguration->mnSamplingRate & 0xff000000) >> 24);
						strlcpy(&p_kBuf[8 + FW_NAME_SIZE], pConfiguration->mpDescription,
								strlen(pConfiguration->mpDescription) + 1);
						ret = copy_to_user(buf, p_kBuf, count);
						if (ret != 0) {
							/* Failed to copy all the data, exit */
							dev_err(pTAS2557->dev, "copy to user fail %d\n", ret);
		...
		case TIAUDIO_CMD_BITRATE: {
			if (g_logEnable)
				dev_info(pTAS2557->dev, "TIAUDIO_CMD_BITRATE: count = %d\n", (int)count);
...
static ssize_t tas2557_file_write(struct file* file, const char* buf, size_t count, loff_t* ppos) {
	...
	switch (pTAS2557->mnDBGCmd) {
		case TIAUDIO_CMD_REG_WITE:
			if (count > 6) {
				chl = p_kBuf[1];
				reg = ((unsigned int)p_kBuf[2] << 24) + ((unsigned int)p_kBuf[3] << 16) +
					  ((unsigned int)p_kBuf[4] << 8) + (unsigned int)p_kBuf[5];
				len = count - 6;
				if (len == 1) {
					ret = pTAS2557->write(pTAS2557, chl, reg, p_kBuf[6]);
					if (g_logEnable)
						dev_info(pTAS2557->dev,
								 "TIAUDIO_CMD_REG_WITE,chn[%d], Reg=0x%x, Val=0x%x\n", chl, reg,
								 p_kBuf[6]);
				} else {
					ret = pTAS2557->bulk_write(pTAS2557, chl, reg, &p_kBuf[6], len);
				}
			} else {
		...
		case TIAUDIO_CMD_REG_READ:
			if (count == 6) {
				...
				pTAS2557->mnCurrentReg = ((unsigned int)p_kBuf[2] << 24) +
										 ...
										 ((unsigned int)p_kBuf[4] << 8) + (unsigned int)p_kBuf[5];
				if (g_logEnable) {
					dev_info(pTAS2557->dev, "TIAUDIO_CMD_REG_READ chl[%d], whole=0x%x\n",
							 pTAS2557->mnCurrentChannel, pTAS2557->mnCurrentReg);
				}
			} else {
		...
		case TIAUDIO_CMD_SAMPLERATE: {
			if (count == 5) {
				unsigned int nSampleRate = ((unsigned int)p_kBuf[1] << 24) +
										   ...
										   ((unsigned int)p_kBuf[3] << 8) + (unsigned int)p_kBuf[4];
				if (g_logEnable)
		...
		case TIAUDIO_CMD_FW_RELOAD: {
			if (count == 1) {
				ret = request_firmware_nowait(THIS_MODULE, 1, TAS2557_FW_NAME, pTAS2557->dev,
