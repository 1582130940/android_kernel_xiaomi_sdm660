				 ...
				 loff_t *ppos)
{
		...
		if (count == 1) {
			...
			if (p_kBuf != NULL) {
				ret = pTAS2557->bulk_read(
					pTAS2557, pTAS2557->mnCurrentChannel,
	...
	case TIAUDIO_CMD_BITRATE: {
		...
		if (count == 1) {
			...
			tas2557_get_bit_rate(
				pTAS2557, pTAS2557->mnCurrentChannel, &bitRate);
	...
	case TIAUDIO_CMD_DACVOLUME: {
		...
		if (count == 1) {
			...
			ret = tas2557_get_DAC_gain(
				pTAS2557, pTAS2557->mnCurrentChannel, &volume);
			if (ret >= 0) {
				  ...
				  size_t count, loff_t *ppos)
{
	...
	case TIAUDIO_CMD_REG_WITE:
		if (count > 6) {
			...
			len = count - 6;
			if (len == 1) {
				ret = pTAS2557->write(pTAS2557, chl, reg,
						      p_kBuf[6]);
				if (g_logEnable)
					dev_info(
						pTAS2557->dev,
				...
				ret = pTAS2557->bulk_write(pTAS2557, chl, reg,
							   &p_kBuf[6], len);
			}
		} else {
			dev_err(pTAS2557->dev,
	...
	case TIAUDIO_CMD_FW_RELOAD: {
		if (count == 1) {
			ret = request_firmware_nowait(
				THIS_MODULE, 1, TAS2557_FW_NAME, pTAS2557->dev,
